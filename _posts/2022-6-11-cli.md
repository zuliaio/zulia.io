...
# flag an index as transient so it loads lazily and can be unloaded when idle
# (requires the transient index settings in zulia.yaml, see the Install docs)
zuliaadmin updateIndex --index someIndex --transientIndex true
```

Common options: `--numberOfReplicas`, `--description`, `--indexWeight`, `--requestFactor`, `--minShardRequest`, `--shardTolerance`, `--shardQueryCacheSize`, `--shardQueryCacheMaxAmount`, `--idleTimeWithoutCommit`, `--shardCommitInterval`, `--ramBufferMB`, `--maxMergeThreads`, `--maxMergePending`, `--indexingThrottle`, `--defaultConcurrency`, `--disableCompression`, `--transientIndex`.
