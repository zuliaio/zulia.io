...
```
cd YOUR_INSTALL_DIR
wget https://github.com/zuliaio/zuliasearch/releases/download/4.13.0/zulia-server-4.13.0.tar
tar xvf zulia-server-4.13.0.tar
rm zulia-server-4.13.0.tar
ln -s zulia-server-4.13.0 zulia-server
```
...
#   systemCpuThresholdPercent: 80.0   # System CPU % above which CPU reports DEGRADED (default: 80.0)
#   jvmCpuThresholdPercent: 90.0      # JVM CPU % above which CPU reports DEGRADED (default: 90.0)
#   writeToMongo: false               # Enable periodic health logging to MongoDB (default: false)
#   db: "zulia"                       # MongoDB database for health logs (default: "zulia")
