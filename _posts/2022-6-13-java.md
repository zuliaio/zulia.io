...
dependencies {
    implementation 'io.zulia:zulia-client:5.5.1'
...
The stored field name is a path into the stored document, with `.` separating levels. Each step reads a key from a document. When a step lands on a list of documents, the key is read from every element and the values are collected into one list. When those elements each hold their own list of documents, the lookup descends into the nested lists and flattens them, so every leaf value is indexed, faceted and sorted.

```groovy
// {"authors": [{"name": "A", "organization": "MIT"}, {"name": "B", "organization": "Oxford"}]}
indexConfig.addFieldConfig(FieldConfigBuilder.createString("authors.organization").index());
// indexes MIT and Oxford

// {"chapters": [{"number": 1, "sections": [{"heading": "A"}, {"heading": "B"}]}, {"number": 2, "sections": [{"heading": "C"}]}]}
indexConfig.addFieldConfig(FieldConfigBuilder.createString("chapters.sections.heading").index());
// indexes A, B and C
```

Elements that lack the key, and null elements, are skipped. An empty string is a value wherever it appears. A leaf value that is itself a list is kept as a list. The same lookup serves the `fl` document field selection on search and the export tools.
...
transientIndex - When true the index loads lazily and can be unloaded from memory when idle or when the node is over its transient cache bound, then reloaded on demand (default false, the index loads at startup and stays resident). See Transient Indexes below.
...
By default every index loads at startup and stays resident in memory. Flagging an index as transient lets it load lazily and unload from memory when it is idle or when the node is over its transient cache bound, then reload on demand. This allows a node to serve many more indexes than fit in memory at once.
...
// or flag an existing index
UpdateIndex updateIndex = new UpdateIndex("someIndex");
updateIndex.setTransientIndex(true);
zuliaWorkPool.updateIndex(updateIndex);
```

The flag alone changes nothing. Lazy loading and eviction are enabled per node by setting `transientIndexCacheSize` or `transientIndexIdleTimeoutSeconds` in `zulia.yaml` (see [Install]({% post_url 2022-6-14-install %}#configuration-reference)). Indexes with replicas are exempt from eviction unless the node sets `transientIndexEvictReplicated: true`. Every operation holds its index through a lease, so eviction always defers to in-flight work.

The flag is also available from the command line: `zuliaadmin updateIndex --index someIndex --transientIndex true`.

The `GetNodes` response reports a per-index `resident` flag plus per-node resident index, load, and eviction counters. A transient index that is currently unloaded shows `resident: false` and carries no shard cache stats.
...
### Doc-Values Skip Index

New fields build a Lucene doc-values skip index by default. The skip index lets range queries, doc-values sorts, and field-exists checks skip over whole blocks of non-matching documents instead of scanning every value, which speeds up these operations on larger indexes. The built-in document-id sort field also gains a skip index in newly created indexes.

A field can opt out by passing `false` to `docValueSkipIndex`:
```groovy
// Default: a skip index is built for this sort field
indexConfig.addFieldConfig(FieldConfigBuilder.createInt("year").index().sort());

// Opt out of the skip index for this field
indexConfig.addFieldConfig(FieldConfigBuilder.createInt("year").index().sort().docValueSkipIndex(false));
```

The flag is frozen once a field exists. Lucene treats the skip index as part of the immutable field schema, so it cannot be toggled on data that is already written. Existing indexes and fields keep working unchanged but do not gain the optimization. To get the benefit on existing data, reindex it into a new index.

### Field Default Values

A field config can carry an index-only default. When the stored path resolves to nothing for a document, because the key is absent or explicitly null, the default is fed into the field's index, sort and facet representations. The stored document is never modified, so a fetch returns exactly what was stored. A field without a default (the norm) behaves exactly as before.

```groovy
indexConfig.addFieldConfig(FieldConfigBuilder.createBool("active").index().facet().defaultValue(false));
indexConfig.addFieldConfig(FieldConfigBuilder.createInt("authorCount").index().sort().defaultValue(0));
...
indexConfig.addFieldConfig(FieldConfigBuilder.createString("journal").indexAs(DefaultAnalyzers.LC_KEYWORD).facet().defaultValue("unknown"));
indexConfig.addFieldConfig(FieldConfigBuilder.createDate("pubDate").index().sort().defaultValue(Date.from(Instant.EPOCH)));
```

With these defaults `active:false` finds documents that omit `active`, `authorCount:[0 TO 10]` finds documents without an author count, and the `journal` facet reports an `unknown` bucket that can be drilled into like any other value.

The default is strongly typed. Each field type takes exactly one Java type and there is no widening: a `NUMERIC_LONG` field takes `0L`, not `0`, and a `NUMERIC_FLOAT` field takes `0.0f`, not `0.0`. A mismatch throws `IllegalArgumentException` from the builder before anything reaches the server, and the server applies the same check to a hand-built config, so `zuliaadmin` and the REST API reject the same mistakes. Vector fields take no default. A geo point field takes a latitude and longitude in degrees, for example `FieldConfigBuilder.createGeoPoint("location").index().defaultValue(38.9072, -77.0369)`.

**Element filling.** By default (`Fill.WHOLE_VALUE`) the default applies only when the document has no value at all for the field. `Fill.EACH_ELEMENT` also fills each list element, or list element sub-field, that is null or absent:

```groovy
indexConfig.addFieldConfig(FieldConfigBuilder.createString("authors.organization")
    .indexAs(DefaultAnalyzers.LC_KEYWORD)
    .facet()
    .defaultValue("unknown", DefaultValue.Fill.EACH_ELEMENT));
```

For `{"authors": [{"name": "A", "organization": "MIT"}, {"name": "B"}, {"name": "C", "organization": "Oxford"}]}` the field indexes `MIT`, `unknown` and `Oxford`, so `authors.organization:unknown` matches, and the `authors.organization` facet counts the document once under `unknown` no matter how many authors lack one. Under `WHOLE_VALUE` the same document indexes only `MIT` and `Oxford`.  Only nulls are filled. An empty string is a value that kept.

Rules to keep in mind:

- **`field:*` keeps meaning "a real value was stored".** A document whose whole value came from the default is not marked as having the field, so `-active:*` still finds the truly missing documents while `active:false` also matches them. Under `EACH_ELEMENT` a document with at least one real element is marked.
- **Sorting.** The default participates in the sort like any stored value, so `missingLast` no longer affects a field with a whole-value default. Pick the default with the sort order in mind.
- **Statistics include them.** Sum, count and percentiles on a numeric field with a default include every defaulted document.
- **Highlighting.** A defaulted document matches but has no stored text to highlight
- **Reindex to apply to existing documents.** Adding or changing a default on update affects documents stored afterwards. Run a reindex to apply it to what is already stored.

Annotated classes use `@DefaultValue`, which is parsed against the declared Java field type. Dates are given as one of the supported date strings.
...
A value that is present but cannot be parsed for the field type, for example `"abc"` in a `NUMERIC_INT` field or `"maybe"` in a `BOOL` field, is a different condition from a missing value and has its own setting. By default (`FAIL`) the document is rejected with a message naming the field and the value, exactly as before. Two lenient modes are available per field:

```groovy
// drop the bad value and index the rest of the document
indexConfig.addFieldConfig(FieldConfigBuilder.createInt("authorCount").index().sort()
    .onMalformed(MalformedValueHandling.SKIP));

// index the default in place of the bad value
indexConfig.addFieldConfig(FieldConfigBuilder.createInt("authorCount").index().sort()
    .defaultValue(-1).onMalformed(MalformedValueHandling.USE_DEFAULT));
```

Both apply per list element, so `[1, "abc", 3]` under `SKIP` indexes `1` and `3` and under `USE_DEFAULT` indexes `1`, `-1` and `3`. `USE_DEFAULT` requires a `defaultValue` on the same field. Vector fields support `FAIL` and `SKIP`. There is no vector default, so `USE_DEFAULT` is rejected for them. Geo point fields support all three. For a vector, malformed means a value that is not a list of numbers, or one whose length disagrees with the declared dimensions. For a geo point, malformed means a value that is not a document, a GeoJSON Point without numeric `[longitude, latitude]` coordinates, a coordinate that is not a number or is out of range, or one coordinate without the other. A geo point document carrying neither coordinate key is absent, not malformed, so a document without coordinates under top-level keys is simply missing.

Lenient handling is never silent. Every document with a skipped or replaced value is marked, and `_zmff_:authorCount` (the stored field name) finds the documents to repair, while `_zmff_:*` finds every document with a malformed value in any field. A typical cleanup sets `SKIP` on the field, reindexes, queries the marker, fixes the documents at the source, and sets the field back to `FAIL`. As with defaults, `field:*` counts only real parsed values, so a document whose every value was skipped or replaced looks missing to `-field:*`.

The same handling covers a value a representation cannot hold. Lucene caps a facet label at 8,191 characters and a sort value at 32,766 bytes. Under `FAIL` such a value rejects the document with a message naming the field, the length and the limit. Under `SKIP` or `USE_DEFAULT` that facet or sort is skipped, the value is still indexed, and the document is marked in `_zmff_` like any other.

Parsing happens once per element, before faceting, sorting and indexing, so all three see the same value and the same decision. One consequence is that a facet-only or sort-only field is exactly as strict as an indexed one. Earlier releases faceted an unparseable numeric string as its raw text and silently skipped an unparseable boolean in the facet and sort paths. Geo point and vector values that could not be parsed used to be skipped silently, or to fail with a raw cast error, and now reject the document under `FAIL` with a message naming the field and the value.

Annotated classes use `@OnMalformed(MalformedValueHandling.SKIP)`.
...
`VectorTopNQuery` accepts the query vector as a `float[]`, a `double[]`, or any `Collection<? extends Number>`, so you can pass whichever vector type you already hold.

#### Vector Quantization

Vector fields are stored through a per-field codec that can quantize vectors on disk, cutting graph-search memory roughly 4x at `INT8` and more at the binary encodings. The supported encodings (`ZuliaIndex.VectorIndexingConfig.Encoding`):

| Encoding | Bits per dimension | Notes |
|----------|--------------------|-------|
| `FLOAT32` | 32 | Raw full-precision vectors. Explicit opt-out of quantization |
| `INT8` | 8 | 8-bit scalar quantization. Default for indexes created on 5.3.0 or later |
| `INT7` | 7 | 7-bit scalar quantization |
| `INT4` | 4 | 4-bit scalar quantization |
| `BBQ` | 1 | Better Binary Quantization |
| `BBQ_2BIT` | 2 | 2-bit binary quantization |

Indexes created on Zulia 5.3.0 or later store vector fields with `INT8` by default unless a field explicitly configures another encoding (including `FLOAT32` to opt out). Indexes created before 5.3.0 keep raw float32 vectors. Each encoding can use an HNSW graph (the default) or a flat exact index.

```groovy
// pin an encoding explicitly
indexConfig.addFieldConfig(FieldConfigBuilder.createUnitVector("v").index().quantization(Encoding.BBQ));

// opt a field out of quantization on a new index
indexConfig.addFieldConfig(FieldConfigBuilder.createUnitVector("v").index().quantization(Encoding.FLOAT32));

// tune the HNSW graph (m = max connections, efConstruction = build beam width; defaults 16 and 100)
indexConfig.addFieldConfig(FieldConfigBuilder.createUnitVector("v").index().quantization(Encoding.INT8).hnsw(24, 200));

// exact brute-force (flat) index instead of the default HNSW graph
indexConfig.addFieldConfig(FieldConfigBuilder.createUnitVector("v").index().flat());
```

A `VectorDescription` on the field config records properties of the vector data itself, shared by every indexed representation of the field:

```groovy
indexConfig.addFieldConfig(FieldConfigBuilder.createVector("embedding")
    .index()
    .dimensions(1024)                          // expected dimensions, validated on store when set
    .similarity(Similarity.MAX_INNER_PRODUCT)  // override (otherwise derived from field type: VECTOR=cosine, UNIT_VECTOR=dot product)
    .model("BAAI/bge-large-en-v1.5"));         // embedding model provenance only, the server does not run inference
```

`Similarity` values (`ZuliaIndex.VectorDescription.Similarity`): `COSINE`, `DOT_PRODUCT`, `EUCLIDEAN`, `MAX_INNER_PRODUCT`.

#### Multiple Vector Representations

One stored vector field can be indexed under multiple names, each with its own encoding and graph settings. This lets a memory-efficient quantized representation sit next to a full-precision twin:

```groovy
indexConfig.addFieldConfig(FieldConfigBuilder.createUnitVector("v")
    .dimensions(384)
    .indexAs("vBBQ", Encoding.BBQ)
    .indexAs("vExact", Encoding.FLOAT32));

// or with full per-representation control via VectorIndexingConfigBuilder
indexConfig.addFieldConfig(FieldConfigBuilder.createUnitVector("v")
    .indexAs("v8", VectorIndexingConfigBuilder.create().quantization(Encoding.INT8).hnsw(24, 200)));
```

#### Oversampling and Rescoring

Queries against quantized fields can oversample candidates and rescore them with full-precision vectors before the cross-shard merge. `oversample(float)` fetches `ceil(topN * oversample)` candidates from the quantized graph, reranks them with full-precision vectors, and keeps the top `topN`:

```groovy
Search search = new Search("vectorTestIndex").setAmount(10);
search.addQuery(new VectorTopNQuery(queryVector, 10, "v").oversample(3.0f));
```

Left unset, the encoding's default applies: `BBQ` oversamples 3x, `BBQ_2BIT` 2x, `INT4` 1.5x, and the remaining encodings do not rescore. A value of `1.0` forces rescoring off. Higher values are capped server-side at 32x.
...
```bash
dependencies {
    implementation 'io.zulia:zulia-ai:5.5.1'
}
```
...
| `BGE_LARGE_EN_V1_5` | 1024 | Asymmetric | Instruction-tuned retrieval (CLS pooling) |
| `GTE_LARGE_EN_V1_5` | 1024 | Symmetric | Retrieval-optimized, 8192 token context (CLS pooling) |
...
| `SPECTER` | 768 | Symmetric | Scientific document similarity (citation-trained), input `title[SEP]abstract` (CLS pooling) |
| `SPECTER2` | 768 | Symmetric | SPECTER2 with proximity adapter, same input convention (CLS pooling) |
...
By default, `addDocumentId(...)` sources are fetched from the queried indexes. Use `addSourceIndex(...)` to fetch them from one or more other indexes instead - the similarity search still runs on the queried indexes, but the source documents are read from the source indexes. This lets you find documents in one index similar to a document that lives in a different index. Source indexes are tried in the order added (first index that has the document wins on collision), and when any source index is given the queried indexes are not used for fetching.

Source-document exclusion (when `setIncludeSourceDocs(false)`, the default) only drops a source document from the results when it could actually be in them: when the source indexes are entirely separate from the queried indexes, the source documents cannot appear in the queried results, so nothing is excluded and an unrelated queried-index document that merely shares a unique ID with a source ID is kept.
...
When multiple sources are given the vectors are averaged into a single query vector. `addLikeVector(...)` accepts a `float[]`, a `double[]`, or any `Collection<? extends Number>`, so you can pass whichever vector type you already hold without converting it first.
...
#### Source Documents in Another Index

```groovy
// search "articles" for documents similar to a document that lives in the "papers" index
Search search = new Search("articles").setAmount(10);
search.addQuery(new MoreLikeThisQuery("title", "abstract")
        .addDocumentId("someDocId")
        .addSourceIndex("papers"));        // fetch the source doc from "papers", not "articles"

// multiple source indexes are tried in order; the first one that has the document wins
Search ordered = new Search("articles").setAmount(10);
ordered.addQuery(new MoreLikeThisQuery("title", "abstract")
        .addDocumentId("someDocId")
        .addSourceIndex("papersPrimary")
        .addSourceIndex("papersArchive"));
```
...
| `setMaxDocFreqPct` | 25 | Ignore terms appearing in more than this percent of a shard's documents (`0` keeps the default of 25, `100` disables the guard; an explicit `setMaxDocFreq` takes precedence) |
...
| `addSourceIndex` | queried indexes | Index(es) to fetch `addDocumentId(...)` source docs from, in priority order |

Notes:
* Lexical term selection ignores terms appearing in more than `maxDocFreqPct` percent of a shard's documents (default 25), so boilerplate and other very common tokens do not dominate. This is on by default: pass `setMaxDocFreqPct(100)` to consider all terms regardless of how common they are.
* If a document ID exists in more than one source index, the first source index in order wins (the source indexes are the queried indexes unless `addSourceIndex(...)` is given).
* In vector-only mode the total result set is capped at `vectorTopN` across all shards, matching `VectorTopNQuery`. Hybrid mode is not capped because lexical matches extend beyond the KNN pool.
