...
dependencies {
    implementation 'io.zulia:zulia-client:4.18.0'
...
A query can target several indexes at once, either by naming them explicitly or with `*` / `?` wildcard patterns. A pattern matches existing index names (not aliases) using glob semantics (`*` = any characters, `?` = a single character) and must match at least one index.
...
```bash
dependencies {
    implementation 'io.zulia:zulia-ai:4.18.0'
}
```
