...
### Store a file
POST the file as multipart form-data (a `file` part, plus an optional `metaJson` part) to the same path used to fetch it:
```
curl -F "file=@readme.txt" -F 'metaJson={"author":"jane"}' \
